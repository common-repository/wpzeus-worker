<?php
/* 
Plugin Name: WPZeus - Worker
Plugin URI: http://www.loopholestudios.co.uk/
Description: Manage all your blogs from one dashboard
Author: Loopholestudios.co.uk
Version: 3.8.8
Author URI: http://www.loopholestudios.co.uk/
*/
...
if (version_compare(PHP_VERSION, '5.0.0', '<')) // min version 5 supported
    exit("<p>WPZeus Worker plugin requires PHP 5 or higher.</p>");
        ...
        if (!get_option('_action_message_id') && !get_option('_worker_public_key')) {
            ...
            if (function_exists('openssl_verify')) {
                ...
                if (!get_option('_worker_nossl_key')) {
                    ...
                    mmb_response('Please deactivate & activate WPZeus Worker plugin on your site, then re-add the site to your dashboard.', false);
            }
        } else {
            mmb_response('Please deactivate & activate WPZeus Worker plugin on your site and re-add the site to your dashboard.', false);
        }
    } else {
...
function mmb_remove_site($params)
{
    ...
    if (!is_plugin_active($plugin_slug))
        mmb_response(array(
            'deactivated' => 'Site removed successfully. <br /><br />WPZeus Worker plugin successfully deactivated.'
        ), true);
    else
        mmb_response(array(
            'removed_data' => 'Site removed successfully. <br /><br /><b>WPZeus Worker plugin was not deactivated.</b>'
        ), true);
